...
<thead><tr><th>ID</th><th>User</th><th>Game</th><th>Stake</th><th>Payout</th><th>Net</th><th>Nonce</th><th>Created</th></tr></thead>
<tbody>
@forelse($entries as $entry)
<tr><td>#{{ $entry->id }}</td><td>{{ $entry->user->email }}</td><td>{{ $entry->game->name }}</td><td>{{ number_format($entry->stake) }}</td><td>{{ number_format($entry->payout) }}</td><td class="{{ $entry->net >= 0 ? 'positive' : 'negative' }}">{{ number_format($entry->net) }}</td><td>{{ $entry->nonce }}</td><td>{{ $entry->created_at->format('Y-m-d H:i:s') }}</td></tr>
@empty
<tr><td colspan="8">No matching entries.</td></tr>
@endforelse
</tbody></table></div>
@if($entries->hasPages())
