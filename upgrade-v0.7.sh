...
printf 'Đang sao lưu SQLite trước khi nâng cấp v0.7...\n'
if [ -f "$APP/database/database.sqlite" ]; then
    cp "$APP/database/database.sqlite" "$APP/database/database.sqlite.backup-v0.7-$(date +%Y%m%d-%H%M%S)"
fi

printf 'Đang chép mã nguồn v0.7...\n'
