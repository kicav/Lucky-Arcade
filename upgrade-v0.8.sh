...
printf 'Đang sao lưu SQLite trước khi nâng cấp v0.8...\n'
if [ -f "$APP/database/database.sqlite" ]; then
    cp "$APP/database/database.sqlite" "$APP/database/database.sqlite.backup-v0.8-$(date +%Y%m%d-%H%M%S)"
fi

printf 'Đang chép mã nguồn v0.8...\n'
...
XDEBUG_MODE=off php artisan arcade:metrics --days=30
XDEBUG_MODE=off php artisan test
XDEBUG_MODE=off php artisan wallets:reconcile
XDEBUG_MODE=off php artisan arcade:backup --keep=14
XDEBUG_MODE=off php artisan arcade:doctor

printf '\nNâng cấp v0.8 hoàn tất. Chạy:\n  bash run-codespaces.sh\n\n'
